...
public class TheGhost extends PacmanItem {
    ...
    private final Queue<Point> prospectivePoints = new LinkedList<>();
    ...
    private final long startPenTime;
    ...
    /**
     * Re-assigns pacman to a point a random distance from Pacman's actual
     * location Use is so that all the ghosts don't go for exactly Pacman
     */
    ...
    /**
     * Checks all 4 directions around the point If that item does not have my
     * number and it's not a wall Add it to the queue and set its number to mine
     * + 1
     */
    ...
    /**
     * Returns the direction to a get to the correct item. Item is based upon
     * game mode
     */
    ...
    private byte itemAtPoint(final Direction theDirection, final Point thePoint) {
        return itemAtPoint(PacmanItem.getNewPoint(thePoint, theDirection));
    }
